						...
						bool use_defaults)
{
	...
	if (resolveCmd != NULL)
		resolved = executeResolveCmd(connstr, resolveCmd);

	if (resolved == NULL)
		resolved = (char *) connstr;

	/* Parse as URI if connection string matches URI prefix */
	if (uri_prefix_length(connstr) != 0)
		return conninfo_uri_parse(resolved, errorMessage, use_defaults);

	/* Parse as default otherwise */
	return conninfo_parse(resolved, errorMessage, use_defaults);
}
